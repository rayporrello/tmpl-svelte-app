...
- **Postgres + Drizzle** — default data layer; starter schema (`contact_submissions`, `automation_events`, `automation_dead_letters`); Drizzle Kit migration workflow; optional self-hosted Postgres Quadlet
...
For the bundled production Postgres path, containers use `DATABASE_URL` with the
project Postgres hostname on the Podman network, while host-side
migrations/backups can use `DATABASE_DIRECT_URL` through the loopback-published
port. Managed Postgres is still fine: set `DATABASE_URL` to the managed service
and leave the bundled Postgres units uninstalled.
