...
Axe accessibility tests run on home, `/articles/sample-post`, and `/styleguide` as part of `bun run test:e2e`. Zero violations are required for the `validate` script to pass.
...
The axe-core integration (`@axe-core/playwright`) runs a full ruleset scan on three pages on every `bun run validate`. Any axe violation blocks the build — this is intentional so accessibility regressions surface at PR time, not in production.
