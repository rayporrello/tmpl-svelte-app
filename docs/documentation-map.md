...
| Design system and semantic HTML      | `src/lib/styles/`, `src/lib/components/Section.svelte`, `src/routes/styleguide/`                                                                                                        | `docs/design-system/`, `ADR-005`, `ADR-008`, `ADR-009`, `ADR-010`                            | `bun run check:design-system`, axe, visual smoke                           |
...
| SEO, discovery, and feeds            | `src/lib/seo/`, `src/lib/components/seo/`, `src/routes/{sitemap.xml,robots.txt,llms.txt,rss.xml}/`                                                                                      | `docs/seo/`, `ADR-011`                                                                       | `bun run check:seo`, `bun run routes:check`, feed/unit/e2e tests           |
| Postgres and Drizzle                 | `src/lib/server/db/`, `drizzle/`, `drizzle.config.ts`, `deploy/quadlets/postgres.*`, `src/routes/readyz/+server.ts`                                                                     | `docs/database/README.md`, `docs/deployment/`, `ADR-004`                                     | `bun run check:db`, deploy preflight, unit tests, bootstrap smoke          |
| Business forms                       | `src/routes/contact/`, `src/lib/forms/`, `src/lib/server/forms/`                                                                                                                        | `docs/forms/README.md`, `docs/design-system/forms-guide.md`                                  | `bun run forms:check`, schema/unit/e2e tests                               |
| Runtime automation outbox            | `src/lib/server/automation/`, `scripts/automation-worker.ts`, `deploy/systemd/automation-worker.*`, `automation_events`, `automation_dead_letters`                                      | `docs/automations/`, `docs/forms/README.md`, `docs/deployment/`, `ADR-015`                   | automation unit tests, `bun run forms:check`, deploy preflight             |
...
| Deployment and runtime               | `.dockerignore`, `Containerfile`, `serve.js`, `deploy/`, `.github/workflows/ci.yml`                                                                                                     | `docs/deployment/`, `ADR-007`, `ADR-018`, `ADR-019`                                          | CI image build, Trivy, container smoke, deploy preflight                   |
| Backups and restore                  | `scripts/backup-*.sh`, `scripts/restore-db.sh`, `scripts/backup-check.ts`, `deploy/systemd/backup.*`                                                                                    | `docs/operations/backups.md`, `docs/operations/restore.md`, `docs/privacy/data-retention.md` | backup unit tests, `bun run backup:verify`, `bun run backup:check`         |
