...
Rules that any automation must follow when writing files to `content/`. Violations break the CMS UI, TypeScript types, and Svelte components simultaneously.

---

## The contract

Content automations must write files that are **structurally identical** to what a human editor would produce through Sveltia CMS. The source of truth for the required structure is `static/admin/config.yml`. n8n is one supported automation operator, not a separate content system.
...
Before enabling a new content automation in production:
