...
| Forms module (Batch D) | sveltekit-superforms v2 added as devDep. EmailProvider seam (interface + console default + postmark.example). In-memory token-bucket rate limiter activated by RATE_LIMIT_ENABLED flag (local guard, not distributed). contact-example route dormant by default (noindex; rename to contact/ to activate). CSP form-action 'self' is correct for same-origin actions; connect-src widening documented for external webhook path. | ACCEPTED | Batch D | — | docs/design-system/forms-guide.md | src/lib/server/forms/, src/lib/forms/contact.schema.ts, src/routes/contact-example/ |