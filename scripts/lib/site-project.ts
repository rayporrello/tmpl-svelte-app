...
export function validateProjectManifest(input: unknown): {
	...
	const rawLoopbackPort = getPath(input, 'deployment.loopbackPort');
	const loopbackPort =
		typeof rawLoopbackPort === 'number'
			? rawLoopbackPort
			: typeof rawLoopbackPort === 'string'
				? Number(rawLoopbackPort)
				: NaN;
	if (!Number.isInteger(loopbackPort) || loopbackPort < 1024 || loopbackPort > 65535) {
		errors.push('deployment.loopbackPort must be an integer between 1024 and 65535.');
	}
...
function rewriteWebmanifest(content: string, manifest: SiteProjectManifest): string {
	...
	// Tab indentation matches the source file and the repo's `.prettierrc`
	// (`useTabs: true`); 2-space JSON would round-trip through `prettier --write`.
...
function rewriteEnvExample(content: string, manifest: SiteProjectManifest): string {
	...
	out = out.replace(/^CLIENT_SLUG=.*/mu, `CLIENT_SLUG=${manifest.project.projectSlug}`);
	out = out.replace(
		/^DATABASE_URL=.*/mu,
		`DATABASE_URL=postgres://${dbSlug}_app_user:replace-me@web-platform-postgres:5432/${dbSlug}_app`
	);
	return out;
}

function rewriteDeployEnvExample(content: string, manifest: SiteProjectManifest): string {
	return rewriteEnvExample(content, manifest);
}
...
function rewriteQuadlet(content: string, manifest: SiteProjectManifest): string {
	...
	out = out.replace(/^Network=.+$/mu, 'Network=web-platform.network');
	out = out.replace(
		/^PublishPort=127\.0\.0\.1:\d+:3000$/mu,
		`PublishPort=127.0.0.1:${manifest.deployment.loopbackPort}:3000`
	);
	out = out.replace(
		/^(HostName=)(.+)$/mu,
...
function buildClaudeAutoBlock(manifest: SiteProjectManifest): string {
	...
	// Trailing blank line before CLAUDE_AUTO_END is required so prettier (markdown
	// parser) does not reflow the block — it expects a blank line between the
	// blockquote and the closing HTML comment. Drift would otherwise show up as
	// LAUNCH-PROJECT-001 against a freshly init'd site.
	return [
		CLAUDE_AUTO_BEGIN,
...
function injectClaudeAutoBlock(content: string, autoBlock: string): string {
	...
	// No trailing \n after autoBlock: content.slice(insertAt) already starts with
	// \n\n (the template's blank line after the H1), so we get exactly one blank
	// line between CLAUDE_AUTO_END and the next block.
	return `${content.slice(0, insertAt)}\n\n${autoBlock}${content.slice(insertAt)}`;
}
